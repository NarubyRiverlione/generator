...
The load breaker is the ship's main power switchgear that connects or disconnects the electrical load from the generator bus in a single instantaneous step. It is not a control-system enable; it models a high-current panel-mount circuit breaker. Its primary teaching purpose is the cold-start load pickup: the operator runs the machine to rated speed and voltage, then closes the breaker to apply the full load in one step, forcing the swing equation and governor to respond.
