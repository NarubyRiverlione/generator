...
- Add a new `ValveDial` SVG component: a circular-bezel indicator with a 270° arc, housing two concentric needles sharing a center pivot.
...
- The dial occupies a standard `gauge-col` (138 px wide) so the switchboard grid needs no layout changes.
...
### New Capabilities
- `valve-dial`: Twin-needle SVG dial showing valve setpoint vs. actual position on a 270° circular face, using the realistic convention of bold-black actual + thin-red setpoint pointer.
...
- **New file**: `src/components/ValveDial.tsx`
- **`src/App.tsx`**: mounts `ValveDial` in the switchboard grid; passes setpoint and actual values as props.
