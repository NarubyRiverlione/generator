...
- [x] 1.1 In `src/core/constants.ts` add: fine-valve→speed band (0 %→47 Hz, 50 %→50 Hz, 100 %→53 Hz;
...
- [x] 1.4 Add `valvePct` (initial 50) and `speedLagged` (initial 1.0 pu) to `SimState` in `src/core/types.ts`
...
- [x] 2.2 Map valve to target speed (linear band) and advance the spin-up lag toward it using the same
...
- [x] 2.4 Derive and return `frequencyHz = 50 × speedLagged`, `rpm = (120 / POLES) × frequencyHz`,
      and `valvePct` in the step output
- [x] 2.5 Ensure initial state (`valvePct = 50`, `speedLagged = 1.0`) leaves default behaviour
      identical to Phase 1 (50 Hz / 1500 rpm at the nominal valve)

## 3. Tests — Simulation Core

- [x] 3.1 Rated speed (nominal valve, speed_pu = 1.0) reproduces the Phase 1 baseline Vₜ, P, Q for given field + load
- [x] 3.2 Valve held lower long enough → `speedLagged` settles so `frequencyHz` ≈ 47 Hz and `rpm` ≈ 1410
- [x] 3.3 Valve step → after one τ_spinup (2.5 s) the lagged speed has moved ~63 % toward the new target
- [x] 3.4 Speed reduction with AVR off causes Vₜ to fall (Eₐ scaled down) — set field high enough that Vₜ stays above the 0.85 relay trip
...
- [x] 6.2 Add a **fine-valve-position** readout (0–100 % of the governing band) so the commanded valve is visible
- [x] 6.3 Update footer text from `PHASE 1 MVP` to `PHASE 2`

## 7. Smoke Test and Polish

- [x] 7.1 Nominal valve (50 %) → 1500 rpm / 50 Hz, no change from Phase 1 visual behaviour
- [x] 7.2 Hold lower → valve %, RPM, and Hz fall together and settle smoothly; Vₜ sags (AVR off)
