...
- [x] 1.1 In `src/core/constants.ts` replace the fine-valve band constants with `VALVE_RPM_MAX = 1600`
...
- [x] 1.4 Set `valvePct` initial value to ~93.1 (giving ~1495 rpm) and `speedLagged` to
...
- [x] 2.2 Map valve to RPM target (linear, shaft-primary) and advance spin-up lag:
...
- [x] 2.4 Derive outputs shaft-first: `rpm = speedLagged × RPM_RATED`, `frequencyHz = rpm / 30`.
...
- [x] 2.5 Verify that initial state (`valvePct ≈ 93.1`, `speedLagged ≈ 0.9967`) starts the sim at
      ~1495 rpm / ~49.8 Hz with Eₐ ≈ 0.9967 × field

## 3. Tests — Simulation Core

- [x] 3.1 Rated-valve scenario: at ~93.75 % valve (settled), `rpm` = 1500 and `frequencyHz` = 50;
      Vₜ, P, Q match the Phase 1 baseline for the same field and load
- [x] 3.2 Low-valve scenario: valve held at, say, 60 % long enough to settle →
      `rpm` ≈ 960, `frequencyHz` ≈ 32 (60 % × 1600 / 30)
- [x] 3.3 Spin-up lag: valve step → after one τ_spinup (2.5 s) lagged speed ~63 % toward new target
...
- [x] 7.1 At initial state (~93.1 % valve) → ~1495 rpm / ~49.8 Hz; raise to ~93.75 % → exactly
